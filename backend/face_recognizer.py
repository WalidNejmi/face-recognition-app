...
def recognize_face(img):
    ...
    for (x, y, w, h) in faces:
        ...
        logging.debug(f"Predicted ID: {id}, Confidence: {confidence}")
        
        if confidence < 100:  # Lower confidence is better in OpenCV
            try:
                name = names[id]
                confidence = round(100 - confidence)  # Convert to a more intuitive scale
            except IndexError:
                name = "Unknown"
                confidence = None
        else:
        ...
        logging.debug(f"Recognized name: {name}, Adjusted confidence: {confidence}")
